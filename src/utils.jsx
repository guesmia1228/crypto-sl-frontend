...
export const encryptData = (password) => {
    try {
        return CryptoJS.AES.encrypt(JSON.stringify(password), process.env.REACT_APP_SECRET_WORD).toString();
    } catch (error) {
...
export const decryptData = (password) => {
    try {
        const bytes = CryptoJS.AES.decrypt(password, process.env.REACT_APP_SECRET_WORD);
